...
TF_model model_load( const char *filename,
        ...
        const char *init_name, const char *train_name, const char *loss_name,
...
static int train_model(TF_model* model, static_storage * const workspace, const reax_system * const system, 
                      int data_size, int window_size, int epoch, int batch_size) {
    fprintf( stdout, "train_model TOP\n" );
    int num_samples = (system->N_cm-2)/2 * (data_size - window_size);
    //int num_samples = (system->N_cm-2) * (data_size - window_size); 
    ...
    get_1diff_training_data(workspace->s, one_diff_training_data, system->N_cm, data_size, window_size, num_samples);
    //get_1diff_training_data(workspace->s_14, one_diff_training_data2, system->N_cm, data_size, window_size, num_samples);
    ...
    for (int i = 0; i < epoch; i++){
      fprintf( stdout, "train_model start first epoch %d\n",i );
...
static int train_and_save(static_storage * const workspace, 
  ...
  printf("data size: %d\n", data_size);
  if (train_model(&model, workspace, system, data_size, window_size, epoch, batch_size)) {
      if(save_model_parameters(&model, checkpoint_prefix)) {
        ...
        simulation_data * const data, static_storage * const workspace )
{
    ...
    if (model.session == NULL) {
        ...
        fprintf(stdout, "%s\n", graph_file );
         ...
         restore_model_parameters(&model,control->cm_init_guess_gd_model);
    ...
    */
    ...
    {
        batch_prediction(&model, i, i + atom_batch, system,
