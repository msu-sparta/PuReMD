...
static int train_model(TF_model* model, static_storage * const workspace, const reax_system * const system, 
    ...
    for (int i = 0; i < epoch; i++){
      ...
      double relative_change = (loss - prev_loss) / prev_loss;
      
      if (relative_change < 0.005 && relative_change > -0.005) {
        fprintf(stdout, "[INFO] Relative change: %.14f \n", relative_change);
      ...
      if (terminate_count == terminate_limit) {
        fprintf( stdout, "[INFO] Early termination\n");
        break;
      }
...
static void load_and_restore_model(TF_model* model, const reax_system * const system,
        const control_params * const control,
        simulation_data * const data, static_storage * const workspace )
{
        ...
        simulation_data * const data, static_storage * const workspace )
{
    ...
    if (model_bot.session == NULL) {
        load_and_restore_model(&model_bot, system, control, data, workspace);
    }
