...
GPU_GLOBAL void k_reset_atomic_forces( rvec * const f,
#if !defined(GPU_STREAM_SINGLE_ACCUM)
        rvec * const f_hb,
#if defined(FUSED_VDW_COULOMB)
        rvec * const f_vdw_clmb,
...
void GPU_Reset_Workspace( reax_system const * const system,
        control_params const * const control, storage * const workspace, cudaStream_t s )
{
    int blocks;
    ...
    k_reset_workspace <<< blocks, control->gpu_block_size, 0, s >>>
...
static void GPU_Reset_Atomic_Forces( reax_system const * const system,
        control_params const * const control, storage * const workspace )
{
    int blocks;
    ...
    k_reset_atomic_forces <<< blocks, control->gpu_block_size, 0, control->gpu_streams[0] >>>
        ( workspace->d_workspace->f,
#if !defined(GPU_STREAM_SINGLE_ACCUM)
          workspace->d_workspace->f_hb,
#if defined(FUSED_VDW_COULOMB)
...
extern "C" void GPU_Reset( reax_system * const system, control_params const * const control,
        simulation_data * const data, storage * const workspace, reax_list ** const lists, int renbr )
{
    if ( system->total_H_atoms > 0 && control->hbond_cut > 0.0 && renbr == TRUE ) {
        GPU_Reset_Atoms_HBond_Indices( system, control, workspace );
    }

    GPU_Reset_Atomic_Forces( system, control, workspace );
