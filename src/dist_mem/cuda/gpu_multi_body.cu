        ...
        output_controls const * const out_control )
{
    ...
    cudaEventRecord( control->gpu_time_events[TE_LPOVUN_START], control->gpu_streams[0] );
#endif
...
#if defined(LOG_PERFORMANCE)
    cudaEventRecord( control->gpu_time_events[TE_LPOVUN_STOP], control->gpu_streams[0] );
#endif
}
