        ...
        output_controls const * const out_control )
{
    ...
    cudaEventRecord( control->gpu_time_events[TE_BOND_ORDER_START], control->gpu_streams[0] );
#endif

    cudaStreamWaitEvent( control->gpu_streams[0], control->gpu_stream_events[SE_INIT_BOND_DONE], 0 );
...
//    cudaCheckError( );
    ...
    cudaEventRecord( control->gpu_stream_events[SE_BOND_ORDER_DONE], control->gpu_streams[0] );

#if defined(LOG_PERFORMANCE)
    cudaEventRecord( control->gpu_time_events[TE_BOND_ORDER_STOP], control->gpu_streams[0] );
#endif
}
