...
extern "C" void GPU_Setup_Environment( reax_system const * const system,
        control_params * const control )
{
    ...
    if ( is_stream_priority_supported == 1 )
    {
        ...
        {
            if ( MAX_GPU_STREAMS - 1 - i < control->num_gpu_streams )
            {
                /* all non-CM gpu_streams of equal priority */
                if ( i != MAX_GPU_STREAMS - 1 )
                {
                    ret = cudaStreamCreateWithPriority( &control->gpu_streams[i], cudaStreamNonBlocking, least_priority );
                }
                /* CM gets highest priority due to MPI comms and cudaMemcpy's */
                else
                {
                    ret = cudaStreamCreateWithPriority( &control->gpu_streams[i], cudaStreamNonBlocking, greatest_priority );
                }
            ...
            {
                control->gpu_streams[i] = control->gpu_streams[MAX_GPU_STREAMS - 1 - ((MAX_GPU_STREAMS - 1 - i) % control->num_gpu_streams)];
            }
        }
    }
    else
    {
        ...
        for ( i = MAX_GPU_STREAMS - 1; i >= 0; --i )
        {
            if ( MAX_GPU_STREAMS - 1 - i < control->num_gpu_streams )
            {
                ret = cudaStreamCreateWithFlags( &control->gpu_streams[i], cudaStreamNonBlocking );
    ...
    {
        ret = cudaEventCreateWithFlags( &control->gpu_stream_events[i], cudaEventDisableTiming );
    ...
    {
        ret = cudaEventCreate( &control->gpu_time_events[i] );
    ...
    ret_cublas = cublasSetStream( control->cublas_handle, control->gpu_streams[5] );
...
extern "C" void GPU_Init_Block_Sizes( reax_system *system,
...
extern "C" void GPU_Cleanup_Environment( control_params const * const control )
{
    int i;
    cudaError_t ret;
#if defined(USE_CUBLAS)
    ...
    for ( i = MAX_GPU_STREAMS - 1; i >= 0; --i )
    {
        if ( MAX_GPU_STREAMS - 1 - i < control->num_gpu_streams )
        {
            ret = cudaStreamDestroy( control->gpu_streams[i] );
    ...
    for ( i = 0; i < GPU_STREAM_SYNC_EVENT_N; ++i )
    {
        ret = cudaEventDestroy( control->gpu_stream_events[i] );
    ...
    for ( i = 0; i < GPU_TIMING_EVENT_N; ++i )
    {
        ret = cudaEventDestroy( control->gpu_time_events[i] );
...
extern "C" void GPU_Print_Mem_Usage( simulation_data const * const data )
{
    int rank;
    size_t total, free;
    cudaError_t ret;
