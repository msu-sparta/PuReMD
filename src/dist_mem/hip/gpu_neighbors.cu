...
GPU_DEVICE static inline real GPU_DistSqr_to_Special_Point( rvec cp, rvec x ) 
{
    int i;  
    real d_sqr = 0.0;
        ...
        int * const realloc, real cutoff2 )
{
    ...
    { 
        ...
        if ( GPU_DistSqr_to_Special_Point(g.nbrs_cp[index_grid_nbrs(i, j, k, itr, &g)], x) <= cutoff )
        {
    ...
    { 
        ...
        if ( GPU_DistSqr_to_Special_Point(g.nbrs_cp[index_grid_nbrs(i, j, k, itr, &g)], x) <= cutoff )
        {
                    ...
                    itr2 < (end - start + warpSize - 1); ++itr2 )
            {
                ...
                num_far = hipcub::ShuffleIndex<WARP_SIZE>( num_far, warpSize - 1, FULL_WARP_MASK );
...
GPU_GLOBAL void k_estimate_neighbors_full_opt( reax_atom const * const my_atoms, 
        ...
        int * const far_nbrs, int * const max_far_nbrs, real cutoff2 )
{
    extern __shared__ hipcub::WarpReduce<int>::TempStorage temp2[];
    ...
    if ( l < N )
    {
        ...
        num_far = hipcub::WarpReduce<int>(temp2[threadIdx.x / warpSize]).Sum(num_far);
    }
    else
    {
        /* used to trigger assignment of max_far_nbrs below */
...
extern "C" int GPU_Generate_Neighbor_Lists( reax_system * const system,
        control_params const * const control, simulation_data * const data,
        storage * const workspace, reax_list ** const lists )
{
    int ret;
#if defined(LOG_PERFORMANCE)
    hipEventRecord( control->gpu_time_events[TE_NBRS_START], control->gpu_streams[0] );
#endif
...
#if defined(LOG_PERFORMANCE)
    hipEventRecord( control->gpu_time_events[TE_NBRS_STOP], control->gpu_streams[0] );
#endif
...
void GPU_Estimate_Num_Neighbors( reax_system * const system,
        control_params const * const control, simulation_data * const data )
{
    int blocks;
#if defined(LOG_PERFORMANCE)
...
//    k_estimate_neighbors_full <<< blocks, control->gpu_block_size, 0, control->gpu_streams[0] >>>
//        ( system->d_my_atoms, system->my_ext_box, system->d_my_grid,
...
//          system->d_far_nbrs, system->d_max_far_nbrs, SQR( control->bond_cut ) );
    ...
    GPU_Reduction_Sum( system->d_max_far_nbrs, system->d_total_far_nbrs,
            system->total_cap, 0, control->gpu_streams[0] );
