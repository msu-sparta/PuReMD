...
//                {
//                    ret = hipStreamCreateWithPriority( &control->gpu_streams[i], hipStreamNonBlocking, least_priority );
//                }
//                /* CM gets highest priority due to MPI comms and hipMemcpy's */
//                else
//                {
//                    ret = hipStreamCreateWithPriority( &control->gpu_streams[i], hipStreamNonBlocking, greatest_priority );
//                }
//        
//                if ( ret != hipSuccess )
//                {
...
//    else
    {
        ...
        for ( i = MAX_GPU_STREAMS - 1; i >= 0; --i )
        {
            if ( MAX_GPU_STREAMS - 1 - i < control->num_gpu_streams )
            {
                ret = hipStreamCreateWithFlags( &control->gpu_streams[i], hipStreamNonBlocking );
    ...
    {
        ret = hipEventCreateWithFlags( &control->gpu_stream_events[i], hipEventDisableTiming );
    ...
    {
        ret = hipEventCreate( &control->gpu_time_events[i] );
    ...
    ret_hipblas = hipblasSetStream( control->hipblas_handle, control->gpu_streams[5] );
...
extern "C" void GPU_Cleanup_Environment( control_params const * const control )
{
    ...
    for ( i = MAX_GPU_STREAMS - 1; i >= 0; --i )
    {
        if ( MAX_GPU_STREAMS - 1 - i < control->num_gpu_streams )
        {
            ret = hipStreamDestroy( control->gpu_streams[i] );
    ...
    for ( i = 0; i < GPU_STREAM_SYNC_EVENT_N; ++i )
    {
        ret = hipEventDestroy( control->gpu_stream_events[i] );
    ...
    for ( i = 0; i < GPU_TIMING_EVENT_N; ++i )
    {
        ret = hipEventDestroy( control->gpu_time_events[i] );
