...
//TODO: move k_jacob and jacboi to gpu_lin_alg.cu
...
//    int ret;
    ...
    switch ( control->cm_solver_pre_comp_type )
    {
        ...
        case SAI_PC:
            ...
            GPU_Copy_Matrix_Device_to_Host( &workspace->H,
        ...
        mpi_datatypes * const mpi_data, hipStream_t s )
{
    ...
    else if ( control->cm_solver_pre_comp_type == SAI_PC )
    {
        ...
        {
            GPU_Allocate_Matrix( &workspace->d_workspace->H_app_inv,
        ...
        {
            GPU_Deallocate_Matrix( &workspace->d_workspace->H_app_inv );
            GPU_Allocate_Matrix( &workspace->d_workspace->H_app_inv,
                    workspace->H_app_inv.n, workspace->H_app_inv.n_max,
                    workspace->H_app_inv.m, workspace->H_app_inv.format, s );
        }

        GPU_Copy_Matrix_Host_to_Device( &workspace->H_app_inv,
        ...
        mpi_datatypes * const mpi_data, hipStream_t s )
{
    ...
    GPU_Reduction_Sum( workspace->d_workspace->x, spad, system->n, 5, s );
    ...
    GPU_Reduction_Sum( workspace->d_workspace->s, &spad[0], system->n, 5, s );
    GPU_Reduction_Sum( workspace->d_workspace->t, &spad[1], system->n, 5, s );
    ...
    Dist( system, mpi_data, workspace->scratch[5], REAL_PTR_TYPE,
            MPI_DOUBLE );
#else
    GPU_Dist( system, workspace, mpi_data, workspace->d_workspace->scratch[5],
...
//    }
    ...
    case CG_S:
#if defined(DUAL_SOLVER)
        iters = GPU_dual_CG( system, control, data, workspace,
                &workspace->d_workspace->H, workspace->d_workspace->b,
                control->cm_solver_q_err, workspace->d_workspace->x, mpi_data,
                refactor, s );
#else
        iters = GPU_CG( system, control, data, workspace, &workspace->d_workspace->H,
    ...
    case SDM_S:
#if defined(DUAL_SOLVER)
        iters = GPU_dual_SDM( system, control, data, workspace,
                &workspace->d_workspace->H, workspace->d_workspace->b,
                control->cm_solver_q_err, workspace->d_workspace->x, mpi_data,
                refactor, s );
#else
        iters = GPU_SDM( system, control, data, workspace, &workspace->d_workspace->H,
                workspace->d_workspace->b_s, control->cm_solver_q_err, workspace->d_workspace->s,
                mpi_data, refactor, s );
        iters += GPU_SDM( system, control, data, workspace, &workspace->d_workspace->H,
                workspace->d_workspace->b_t, control->cm_solver_q_err, workspace->d_workspace->t,
    ...
    case BiCGStab_S:
#if defined(DUAL_SOLVER)
        iters = GPU_dual_BiCGStab( system, control, data, workspace,
                &workspace->d_workspace->H, workspace->d_workspace->b,
                control->cm_solver_q_err, workspace->d_workspace->x, mpi_data,
                refactor, s );
#else
        iters = GPU_BiCGStab( system, control, data, workspace, &workspace->d_workspace->H,
                workspace->d_workspace->b_s, control->cm_solver_q_err, workspace->d_workspace->s,
                mpi_data, refactor, s );
        iters += GPU_BiCGStab( system, control, data, workspace, &workspace->d_workspace->H,
                workspace->d_workspace->b_t, control->cm_solver_q_err, workspace->d_workspace->t,
    ...
    case PIPECG_S:
#if defined(DUAL_SOLVER)
        iters = GPU_dual_PIPECG( system, control, data, workspace,
                &workspace->d_workspace->H, workspace->d_workspace->b,
                control->cm_solver_q_err, workspace->d_workspace->x, mpi_data,
                refactor, s );
#else
        iters = GPU_PIPECG( system, control, data, workspace, &workspace->d_workspace->H,
                workspace->d_workspace->b_s, control->cm_solver_q_err, workspace->d_workspace->s,
                mpi_data, refactor, s );
        iters += GPU_PIPECG( system, control, data, workspace, &workspace->d_workspace->H,
                workspace->d_workspace->b_t, control->cm_solver_q_err, workspace->d_workspace->t,
    ...
    case PIPECR_S:
#if defined(DUAL_SOLVER)
        iters = GPU_dual_PIPECR( system, control, data, workspace,
                &workspace->d_workspace->H, workspace->d_workspace->b,
                control->cm_solver_q_err, workspace->d_workspace->x, mpi_data,
                refactor, s );
#else
        iters = GPU_PIPECR( system, control, data, workspace, &workspace->d_workspace->H,
                workspace->d_workspace->b_s, control->cm_solver_q_err, workspace->d_workspace->s,
                mpi_data, refactor, s );
        iters += GPU_PIPECR( system, control, data, workspace, &workspace->d_workspace->H,
                workspace->d_workspace->b_t, control->cm_solver_q_err, workspace->d_workspace->t,
...
void GPU_Compute_Charges( reax_system const * const system,
        control_params const * const control,
