...
//    hipCheckError( );
    ...
    sHipMemcpyAsync( &system->total_thbodies, system->d_total_thbodies,
            sizeof(int), hipMemcpyDeviceToHost, control->gpu_streams[3], __FILE__, __LINE__ );
    hipStreamSynchronize( control->gpu_streams[3] );
        ...
        reax_list **lists, output_controls const * const out_control )
{
    ...
    hipEventRecord( control->gpu_time_events[TE_VALENCE_START], control->gpu_streams[3] );
#endif
    ...
    hipStreamWaitEvent( control->gpu_streams[3], control->gpu_stream_events[SE_BOND_ORDER_DONE], 0 );
...
#if defined(GPU_ATOMIC_EV)
        sHipMemsetAsync( &data->d_my_en[E_ANG], 0, sizeof(real) * 3,
                control->gpu_streams[3], __FILE__, __LINE__ );
        if ( control->virial == 1 )
        {
            sHipMemsetAsync( &data->d_my_ext_press,
...
#if defined(LOG_PERFORMANCE)
    hipEventRecord( control->gpu_time_events[TE_VALENCE_STOP], control->gpu_streams[3] );
#endif
