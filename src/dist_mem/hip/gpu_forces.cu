        ...
        int realloc_cm, int realloc_bonds, int realloc_hbonds, int step )
{
    ...
    if ( realloc_cm == TRUE ) {
#if defined(LOG_PERFORMANCE)
        hipEventRecord( control->gpu_time_events[TE_INIT_CM_START], control->gpu_streams[5] );
#endif
...
//                  system->d_cm_entries, system->d_max_cm_entries );
        ...
        sHipMemcpyAsync( &system->total_cm_entries, system->d_total_cm_entries,
                sizeof(int), hipMemcpyDeviceToHost, control->gpu_streams[5], __FILE__, __LINE__ );

#if defined(LOG_PERFORMANCE)
        hipEventRecord( control->gpu_time_events[TE_INIT_CM_STOP], control->gpu_streams[5] );
#endif
    }

    if ( realloc_bonds == TRUE ) {
#if defined(LOG_PERFORMANCE)
        hipEventRecord( control->gpu_time_events[TE_INIT_BOND_START], control->gpu_streams[1] );
#endif
...
//              system->d_bonds, system->d_max_bonds );
        ...
        sHipMemcpyAsync( &system->total_bonds, system->d_total_bonds, sizeof(int), 
                hipMemcpyDeviceToHost, control->gpu_streams[1], __FILE__, __LINE__ );

#if defined(LOG_PERFORMANCE)
        hipEventRecord( control->gpu_time_events[TE_INIT_BOND_STOP], control->gpu_streams[1] );
#endif
    }

    if ( system->total_H_atoms > 0 && control->hbond_cut > 0.0 && realloc_hbonds == TRUE ) {
#if defined(LOG_PERFORMANCE)
        hipEventRecord( control->gpu_time_events[TE_INIT_HBOND_START], control->gpu_streams[2] );
#endif
...
//              system->d_hbonds, system->d_max_hbonds );
        ...
        sHipMemcpyAsync( &system->total_hbonds, system->d_total_hbonds, sizeof(int), 
                hipMemcpyDeviceToHost, control->gpu_streams[2], __FILE__, __LINE__ );

#if defined(LOG_PERFORMANCE)
        hipEventRecord( control->gpu_time_events[TE_INIT_HBOND_STOP], control->gpu_streams[2] );
#endif
    }

    if ( realloc_cm == TRUE ) {
        hipStreamSynchronize( control->gpu_streams[5] );

#if defined(LOG_PERFORMANCE)
        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_INIT_CM_START],
                control->gpu_time_events[TE_INIT_CM_STOP] ); 
        data->timing.init_cm += (real) (time_elapsed / 1000.0);
#endif
    }
    if ( realloc_bonds == TRUE ) {
        hipStreamSynchronize( control->gpu_streams[1] );

#if defined(LOG_PERFORMANCE)
        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_INIT_BOND_START],
                control->gpu_time_events[TE_INIT_BOND_STOP] ); 
        data->timing.init_bond += (real) (time_elapsed / 1000.0);
#endif
    }
    if ( system->total_H_atoms > 0 && control->hbond_cut > 0.0 && realloc_hbonds == TRUE ) {
        hipStreamSynchronize( control->gpu_streams[2] );

#if defined(LOG_PERFORMANCE)
        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_INIT_HBOND_START],
...
#if defined(LOG_PERFORMANCE)
        hipEventRecord( control->gpu_time_events[TE_INIT_DIST_START], control->gpu_streams[0] );
#endif
...
//            ( system->d_my_atoms, *(lists[FAR_NBRS]), system->N );
        ...
        hipEventRecord( control->gpu_stream_events[SE_INIT_DIST_DONE], control->gpu_streams[0] );

#if defined(LOG_PERFORMANCE)
        hipEventRecord( control->gpu_time_events[TE_INIT_DIST_STOP], control->gpu_streams[0] );
#endif
    ...
    if ( cm_done == FALSE ) {
        ...
        if ( renbr == FALSE ) {
            hipStreamWaitEvent( control->gpu_streams[5], control->gpu_stream_events[SE_INIT_DIST_DONE], 0 );
        }
...
//                  workspace->d_workspace->H.n_max );
    ...
    if ( bonds_done == FALSE ) {
        ...
        GPU_Reset_Workspace( system, control, workspace, control->gpu_streams[1] );

        if ( renbr == FALSE ) {
            hipStreamWaitEvent( control->gpu_streams[1], control->gpu_stream_events[SE_INIT_DIST_DONE], 0 );
        }
...
//        hipCheckError( );
    ...
    if ( system->total_H_atoms > 0 && control->hbond_cut > 0.0 && hbonds_done == FALSE ) {
        ...
        if ( renbr == FALSE ) {
            hipStreamWaitEvent( control->gpu_streams[2], control->gpu_stream_events[SE_INIT_DIST_DONE], 0 );
        }
...
//        hipCheckError( );
    ...
    if ( cm_done == FALSE ) {
        sHipMemcpyAsync( &workspace->realloc[RE_CM],
                ...
                hipMemcpyDeviceToHost, control->gpu_streams[5], __FILE__, __LINE__ );
    } else {
...
//            ( *(lists[BONDS]), system->N );
        ...
        hipEventRecord( control->gpu_stream_events[SE_INIT_BOND_DONE], control->gpu_streams[1] );
...
{
    sHipHostAllocCheck( &workspace->scratch[0], &workspace->scratch_size[0],
        ...
        output_controls * const out_control, mpi_datatypes * const mpi_data )
{
    ...
    if ( ret == SUCCESS ) {
        ...
        for ( i = 0; i < MAX_GPU_STREAMS; ++i ) {
            hipStreamSynchronize( control->gpu_streams[i] );
        }
        ...
        if ( (data->step - data->prev_steps) % control->reneighbor == 0 ) {
            hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_NBRS_START],
                    control->gpu_time_events[TE_NBRS_STOP] ); 
            data->timing.nbrs += (real) (time_elapsed / 1000.0);

            hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_INIT_CM_START],
                    control->gpu_time_events[TE_INIT_CM_STOP] ); 
            hipEventElapsedTime( &time_elapsed2, control->gpu_time_events[TE_INIT_CM_START],
                    control->gpu_time_events[TE_INIT_BOND_STOP] ); 
            if ( system->total_H_atoms > 0 && control->hbond_cut > 0.0 ) {
                hipEventElapsedTime( &time_elapsed3, control->gpu_time_events[TE_INIT_CM_START],
                        control->gpu_time_events[TE_INIT_HBOND_STOP] ); 
            } else {
                time_elapsed3 = 0.0;
            }
            hipEventElapsedTime( &time_elapsed4, control->gpu_time_events[TE_INIT_BOND_START],
                    control->gpu_time_events[TE_INIT_CM_STOP] ); 
            hipEventElapsedTime( &time_elapsed5, control->gpu_time_events[TE_INIT_BOND_START],
                    control->gpu_time_events[TE_INIT_BOND_STOP] ); 
            if ( system->total_H_atoms > 0 && control->hbond_cut > 0.0 ) {
                hipEventElapsedTime( &time_elapsed6, control->gpu_time_events[TE_INIT_BOND_START],
                        control->gpu_time_events[TE_INIT_HBOND_STOP] ); 
                hipEventElapsedTime( &time_elapsed7, control->gpu_time_events[TE_INIT_HBOND_START],
                        control->gpu_time_events[TE_INIT_CM_STOP] ); 
                hipEventElapsedTime( &time_elapsed8, control->gpu_time_events[TE_INIT_HBOND_START],
                        control->gpu_time_events[TE_INIT_BOND_STOP] ); 
                hipEventElapsedTime( &time_elapsed9, control->gpu_time_events[TE_INIT_HBOND_START],
                        control->gpu_time_events[TE_INIT_HBOND_STOP] ); 
            } else {
                time_elapsed6 = 0.0;
                time_elapsed7 = 0.0;
            ...
            hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_INIT_DIST_START],
                    control->gpu_time_events[TE_INIT_CM_STOP] ); 
            hipEventElapsedTime( &time_elapsed2, control->gpu_time_events[TE_INIT_DIST_START],
                    control->gpu_time_events[TE_INIT_BOND_STOP] ); 
            if ( system->total_H_atoms > 0 && control->hbond_cut > 0.0 ) {
                hipEventElapsedTime( &time_elapsed3, control->gpu_time_events[TE_INIT_DIST_START],
                        control->gpu_time_events[TE_INIT_HBOND_STOP] ); 
            } else {
                time_elapsed3 = 0.0;
            }
        ...
        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_BOND_ORDER_START],
                control->gpu_time_events[TE_LPOVUN_STOP] ); 
        hipEventElapsedTime( &time_elapsed2, control->gpu_time_events[TE_BOND_ORDER_START],
                control->gpu_time_events[TE_BONDS_STOP] ); 
        hipEventElapsedTime( &time_elapsed3, control->gpu_time_events[TE_BOND_ORDER_START],
                control->gpu_time_events[TE_TORSION_STOP] ); 
        if ( system->total_H_atoms > 0 && control->hbond_cut > 0.0 ) {
            hipEventElapsedTime( &time_elapsed4, control->gpu_time_events[TE_BOND_ORDER_START],
        ...
        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_BONDS_START],
                control->gpu_time_events[TE_BONDS_STOP] ); 
        data->timing.bonds += (real) (time_elapsed / 1000.0);

        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_LPOVUN_START],
                control->gpu_time_events[TE_LPOVUN_STOP] ); 
        data->timing.lpovun += (real) (time_elapsed / 1000.0);

        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_VALENCE_START],
                control->gpu_time_events[TE_VALENCE_STOP] ); 
        data->timing.valence += (real) (time_elapsed / 1000.0);

        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_TORSION_START],
                control->gpu_time_events[TE_TORSION_STOP] ); 
        data->timing.torsion += (real) (time_elapsed / 1000.0);

        if ( system->total_H_atoms > 0 && control->hbond_cut > 0.0 ) {
            hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_HBONDS_START],
                    control->gpu_time_events[TE_HBONDS_STOP] ); 
            data->timing.hbonds += (real) (time_elapsed / 1000.0);
        }

#if !defined(FUSED_VDW_COULOMB)
        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_VDW_START],
                control->gpu_time_events[TE_VDW_STOP] ); 
        data->timing.nonb += (real) (time_elapsed / 1000.0);
#endif

        hipEventElapsedTime( &time_elapsed, control->gpu_time_events[TE_COULOMB_START],
