...
#define BL (bond_list.bond_list_gpu)
    ...
    /* correction for C2 */
    if ( gp_l[5] > 0.001
            && GPU_strncmp( sbp[type_i].name, "C", sizeof(sbp[type_i].name) ) == 0 )
    {
        for ( pj = Start_Index(i, &bond_list); pj < End_Index(i, &bond_list); ++pj )
        {
                    ...
                    my_atoms[BL.nbr[pj]].orig_id )
            {
                ...
                if ( GPU_strncmp( sbp[type_j].name, "C", sizeof(sbp[type_j].name) ) == 0 )
                {
                    Di = Delta[i];
...
void GPU_Compute_Atom_Energy( reax_system const * const system,
        ...
        output_controls const * const out_control )
{
    ...
    hipEventRecord( control->gpu_time_events[TE_LPOVUN_START], control->gpu_streams[0] );
#endif

#if defined(GPU_ATOMIC_EV)
    sHipMemsetAsync( &data->d_my_en[E_OV], 0, sizeof(real) * 3,
...
//    hipCheckError( );
    ...
    {
        GPU_Reduction_Sum( spad, &data->d_my_en[E_LP], system->n, 0,
                control->gpu_streams[0] );

        GPU_Reduction_Sum( &spad[system->n], &data->d_my_en[E_OV], system->n, 0,
                control->gpu_streams[0] );

        GPU_Reduction_Sum( &spad[2 * system->n], &data->d_my_en[E_UN], system->n, 0,
                control->gpu_streams[0] );
    }
#endif

#if defined(LOG_PERFORMANCE)
    hipEventRecord( control->gpu_time_events[TE_LPOVUN_STOP], control->gpu_streams[0] );
#endif
}
