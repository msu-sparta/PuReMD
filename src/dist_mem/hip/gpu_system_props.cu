...
static void GPU_Compute_Momentum( reax_system * const system,
        ...
        storage * const workspace, rvec xcm, rvec vcm, rvec amcm )
{
...
#if !defined(GPU_KERNEL_ATOMIC)
    GPU_Reduction_Sum( spad, &spad[blocks], blocks, 0, control->gpu_streams[0] );
#endif
    ...
    sHipMemcpyAsync( xcm,
            ...
            &spad[blocks],
#endif
            sizeof(rvec), hipMemcpyDeviceToHost, control->gpu_streams[0],
            __FILE__, __LINE__ );
...
static void GPU_Compute_Inertial_Tensor( reax_system * const system,
        ...
        real * const t, rvec my_xcm )
{
    ...
    sHipMemcpyAsync( t, &spad[6 * control->blocks_n],
...
extern "C" void GPU_Compute_Kinetic_Energy( reax_system * const system,
        control_params const * const control, storage * const workspace,
        simulation_data * const data, MPI_Comm comm )
{
    ...
    sHipMemcpyAsync( &data->my_en[E_KIN], &kinetic_energy[system->n],
            sizeof(real), hipMemcpyDeviceToHost, control->gpu_streams[0],
            __FILE__, __LINE__ );
    hipStreamSynchronize( control->gpu_streams[0] );
...
void GPU_Compute_Total_Mass( reax_system * const system,
        control_params const * const control,
        storage * const workspace, simulation_data * const data, MPI_Comm comm )
{
    ...
    GPU_Reduction_Sum( spad, &spad[system->n], system->n, 0, control->gpu_streams[0] );
...
extern "C" void GPU_Compute_Center_of_Mass( reax_system * const system,
        control_params const * const control, storage * const workspace,
        ...
        MPI_Comm comm )
{
    ...
    GPU_Compute_Momentum( system, control, workspace, my_xcm, my_vcm, my_amcm );
    ...
    GPU_Compute_Inertial_Tensor( system, control, workspace, tmp_mat, data->xcm );
 ...
 */
void GPU_Compute_Pressure( reax_system * const system,
        control_params const * const control,
        ...
        mpi_datatypes * const mpi_data )
{
    ...
    if ( control->press_mode == 0 || control->press_mode == 2 )
    {
        ...
        GPU_Reduction_Sum( spad_rvec, &spad_rvec[system->n], system->n,
                0, control->gpu_streams[0] );
