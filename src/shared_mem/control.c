        ...
        output_controls * const out_control )
{
    ...
    if ( strncmp(keyword, "simulation_name", MAX_LINE) == 0 ) {
        ...
        out_control->restart_format = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "restart_freq", MAX_LINE) == 0 ) {
        out_control->restart_freq = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "random_vel", MAX_LINE) == 0 ) {
        control->random_vel = (bool) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "reposition_atoms", MAX_LINE) == 0 ) {
        control->reposition_atoms = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "ensemble_type", MAX_LINE) == 0 ) {
        control->ensemble = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "nsteps", MAX_LINE) == 0 ) {
        control->nsteps = sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        control->periodic_boundaries = (bool) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "geo_format", MAX_LINE) == 0 ) {
        control->geo_format = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "restrict_bonds", MAX_LINE) == 0 ) {
        control->restrict_bonds = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "tabulate_long_range", MAX_LINE) == 0 ) {
        control->tabulate = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "reneighbor", MAX_LINE) == 0 ) {
        control->reneighbor = sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        control->charge_method = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "charge_freq", MAX_LINE) == 0 ) {
        control->charge_freq = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "cm_q_net", MAX_LINE) == 0 ) {
        val = sstrtod( values[0], __FILE__, __LINE__ );
        control->cm_q_net = val;
    } else if ( strncmp(keyword, "cm_solver_type", MAX_LINE) == 0 ) {
        control->cm_solver_type = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "cm_solver_max_iters", MAX_LINE) == 0 ) {
        control->cm_solver_max_iters = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "cm_solver_restart", MAX_LINE) == 0 ) {
        control->cm_solver_restart = sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        control->cm_init_guess_extrap1 = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "cm_init_guess_extrap2", MAX_LINE) == 0 ) {
        control->cm_init_guess_extrap2 = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        control->cm_solver_pre_comp_type = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        control->cm_solver_pre_comp_sweeps = sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        control->cm_solver_pre_app_type = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "cm_solver_pre_app_jacobi_iters", MAX_LINE) == 0 ) {
        control->cm_solver_pre_app_jacobi_iters = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "include_polarization_energy", MAX_LINE) == 0 ) {
        control->polarization_energy_enabled = (bool) sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        control->T_mode = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        control->press_mode = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "compute_pressure", MAX_LINE) == 0 ) {
        control->compute_pressure = (bool) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "remove_CoM_vel", MAX_LINE) == 0 ) {
        control->remove_CoM_vel = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "energy_update_freq", MAX_LINE) == 0 ) {
        out_control->log_update_freq = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "write_freq", MAX_LINE) == 0 ) {
        out_control->write_steps = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "traj_compress", MAX_LINE) == 0 ) {
        out_control->traj_compress = sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        out_control->traj_format = sstrtoul( values[0], __FILE__, __LINE__ );
        ...
        out_control->atom_format +=
            sstrtoul( values[0], __FILE__, __LINE__ ) * 4;
    } else if ( strncmp(keyword, "atom_velocities", MAX_LINE) == 0 ) {
        out_control->atom_format +=
            sstrtoul( values[0], __FILE__, __LINE__ ) * 2;
    } else if ( strncmp(keyword, "atom_forces", MAX_LINE) == 0 ) {
        out_control->atom_format +=
            sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "bond_info", MAX_LINE) == 0 ) {
        out_control->bond_info = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "angle_info", MAX_LINE) == 0 ) {
        out_control->angle_info = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "test_forces", MAX_LINE) == 0 ) {
        ;
    } else if ( strncmp(keyword, "molec_anal", MAX_LINE) == 0 ) {
        control->molec_anal = (uint8_t) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "freq_molec_anal", MAX_LINE) == 0 ) {
        control->freq_molec_anal = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "bond_graph_cutoff", MAX_LINE) == 0 ) {
        val = sstrtod( values[0], __FILE__, __LINE__ );
        control->bg_cut = val;
    } else if ( strncmp(keyword, "ignore", MAX_LINE) == 0 ) {
        control->num_ignored = sstrtoul( values[0], __FILE__, __LINE__ );
        for ( i = 0; i < control->num_ignored; ++i )
            control->ignore[ sstrtoul( values[i + 1], __FILE__, __LINE__ ) ] = 1;
    } else if ( strncmp(keyword, "dipole_anal", MAX_LINE) == 0 ) {
        control->dipole_anal = (bool) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "freq_dipole_anal", MAX_LINE) == 0 ) {
        control->freq_dipole_anal = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "diffusion_coef", MAX_LINE) == 0 ) {
        control->diffusion_coef = (bool) sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "freq_diffusion_coef", MAX_LINE) == 0 ) {
        control->freq_diffusion_coef = sstrtoul( values[0], __FILE__, __LINE__ );
    } else if ( strncmp(keyword, "restrict_type", MAX_LINE) == 0 ) {
        control->restrict_type = sstrtoul( values[0], __FILE__, __LINE__ );
    } else {
        ret = FAILURE;
    }
