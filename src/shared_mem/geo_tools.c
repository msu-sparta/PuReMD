...
#define CUSTOM_ATOM_FORMAT " %u %s %s %lf %lf %lf"
...
#define PDB_ATOM_FORMAT "%6s%5u%4s%c%4s%c%4u%c%8s%8s%8s%6s%6s%4s%2s%2s\n"
#define PDB_ATOM_FORMAT_LENGTH (72)
#define PDB_HETATM_FORMAT "%6s%5u%4s%c%4s%c%4u%c%8s%8s%8s%6s%6s%2s%2s\n"
#define PDB_CONECT_FORMAT "%6s%5u%5u%5u%5u%5u\n"
#define PDB_CRYST1_FORMAT "%6s%9s%9s%9s%7s%7s%7s%11s%4s\n"

#define PDB_ATOM_FORMAT_O "%6s%5u %4s%c%3s %c%4u%c   %8.3f%8.3f%8.3f%6.2f%6.2f          %2s%2s\n"
#define PDB_CRYST1_FORMAT_O "%6s%9.3f%9.3f%9.3f%7.2f%7.2f%7.2f%11s%4u\n"

#define BGF_ATOM_FORMAT "%6s %5s %5s %3s %c %5s%10s%10s%10s %5s%3s%2s %8s"
#define BGF_CRYSTX_FORMAT "%8s%11s%11s%11s%11s%11s%11s"

#define BGF_ATOM_FORMAT_O "%6s %5u %-5s %3s %c %5s%10.5f%10.5f%10.5f %-5s%3u%2u %8.5f"
 ...
 * */
static int32_t Read_Box_Info( reax_system *system, FILE *fp, uint8_t geo_format )
{
    int32_t ret;
    size_t cryst_len;
    char *cryst, line[MAX_LINE], descriptor[9];
    ...
    switch ( geo_format ) {
        ...
        default:
            fprintf( stderr, "[ERROR] Unknown geometry file format (%u). Terminating...\n",
                  geo_format );
            break;
    }
    if ( ferror( fp ) ) {
        ...
        simulation_data *data, static_storage *workspace )
{
    ...
    for ( i = 0; i < system->N; ++i ) {
        if ( fscanf( geo, CUSTOM_ATOM_FORMAT,
                    &serial, element, name, &x[0], &x[1], &x[2] ) != 6 ) {
            fprintf( stderr, "[ERROR] reading geometry file failed\n" \
                     "  [INFO] reading atom info (entry %u)\n", i );
            exit( INVALID_INPUT );
        }
...
void Read_PDB( const char * const pdb_file, reax_system* system, control_params *control,
        simulation_data *data, static_storage *workspace )
{
    uint32_t i, n, c, c1, pdb_serial, top;
    FILE *pdb;
    char **tmp;
    char *s, *s1;
...
//                icode = s1[26];
            ...
            if ( is_Inside_Box( &system->box, x ) ) {
                ...
                atom = &system->atoms[top];
                pdb_serial = sstrtoul( serial, __FILE__, __LINE__ );
        ...
        control_params *control, static_storage *workspace, output_controls *out_control )
{
    ...
    /* write header */
    snprintf( fname, sizeof(fname) - 1, "%.*s-%.*u.pdb",
            (uint32_t) MIN( strnlen(control->sim_name,
                    sizeof(fname) - snprintf(NULL, 0, "%u", data->step) - 6 ),
                sizeof(control->sim_name) ),
            control->sim_name, snprintf(NULL, 0, "%u", data->step), data->step );
...
#if defined(DEBUG_FOCUS)
    fprintf( stderr, "[INFO] num_atoms = %u, num_mcc = %u\n", system->N,
...
//                fprintf( stderr, "[ERROR] reading geometry file failed\n" );
//                fprintf( stderr, "  [INFO] reading atom info (entry %u)\n", i );
//                exit( INVALID_INPUT );
//            }

            /* add to mapping */
            bgf_serial = sstrtoul( serial, __FILE__, __LINE__ );
...
#if defined(DEBUG_FOCUS)
            fprintf( stderr,
                    "[INFO] atom_cnt = %5u, atom_type = %3u, x = (%10.5f,%10.5f,%10.5f), q = %10.5f, occ = %s, temp = %s, res_name = %4s, element = %s\n",
            ...
            system->molec_charge_constraints[num_mcc] = sstrtoul( tokens[3], __FILE__, __LINE__ );

#if defined(DEBUG_FOCUS)
            fprintf( stderr,
                    "[INFO] num_mcc = %u, mcc = %f, mcc_range = (%u, %u)\n", num_mcc,
