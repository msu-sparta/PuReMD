...
{
    uint32_t atom_count;
    uint32_t atom_list[MAX_MOLECULE_SIZE];
    uint32_t mtypes[MAX_ATOM_TYPES];
} molecule;
...
static void Get_Molecule( uint32_t atom, molecule *m, bool *mark, reax_system *system,
        control_params *control, reax_list *bonds, bool print, FILE *fout )
{
    ...
    for ( i = start; i < end; ++i ) {
        if ( bonds->bond_list[i].bo_data.BO >= control->bg_cut
                && mark[bonds->bond_list[i].nbr] == FALSE ) {
...
{
    uint32_t atom, i, j, k, l;
    bool flag;
    bool *mark = workspace->mark;
    bool *old_mark = workspace->old_mark;
        ...
        if ( mark[atom] == FALSE ) {
            ...
            if ( Compare_Bond_Lists( atom, control, lists ) ) {
                ...
                Get_Molecule( atom, &old_molecules[num_old++], old_mark,
                              system, control, old_bonds, true, fout );
                fprintf( fout, "\nget_new_mol: " );
                Get_Molecule( atom, &new_molecules[num_new++], mark,
                              system, control, new_bonds, true, fout );
                fprintf( fout, "\n" );
                ...
                while ( flag == false ) {
                    ...
                    for ( ; k < num_new; ++k )
                        for ( l = 0; l < new_molecules[k].atom_count; ++l )
                            if ( old_mark[new_molecules[k].atom_list[l]] == FALSE ) {
                                fprintf( fout, "get_old_mol: " );
                                Get_Molecule( new_molecules[k].atom_list[l],
                                              &old_molecules[num_old++],
                                              old_mark, system, control, old_bonds, true, fout );
                                fprintf( fout, "\n" );
                                flag = false;
                            }
                }
...
static void Report_Bond_Change( reax_system *system, control_params *control,
        ...
        reax_list *new_bonds, int32_t a1, int32_t a2, bool flag, FILE *fout )
{
    uint32_t i;
    int32_t rev1, rev2;
            ...
            if ( newp < End_Index( atom, new_bonds ) ) {
                if ( old_bonds->bond_list[oldp].nbr
                    ...
                    Report_Bond_Change( system, control, workspace, old_bonds, new_bonds,
                                        atom, old_bonds->bond_list[oldp].nbr, false,
                                        fout );
                    ++oldp;
                    ...
                    Report_Bond_Change( system, control, workspace, old_bonds, new_bonds,
                                        atom, new_bonds->bond_list[newp].nbr, true,
                                        fout );
                    ++newp;
                } else {
                    ++newp;
                ...
                while ( oldp < End_Index( atom, old_bonds ) ) {
                    if ( old_bonds->bond_list[oldp].bo_data.BO >= control->bg_cut ) {
                        ...
                        Report_Bond_Change( system, control, workspace,
                                            ...
                                            old_bonds->bond_list[oldp].nbr, false,
                                            fout );
                    }
                    ++oldp;
                }
            ...
            if ( newp < End_Index( atom, new_bonds ) ) {
                ...
                while ( newp < End_Index( atom, new_bonds ) ) {
                    if ( new_bonds->bond_list[newp].bo_data.BO >= control->bg_cut ) {
                        ...
                        Report_Bond_Change( system, control, workspace,
                                old_bonds, new_bonds, atom,
                                new_bonds->bond_list[newp].nbr, true, fout );
                    }
                    ++newp;
                }
            } else {
...
static void Visit_Bonds( uint32_t atom, bool *mark, uint32_t *type, reax_system *system,
                  control_params *control, reax_list *bonds, bool ignore )
{
    uint32_t i, t, start, end, nbr;
    real bo;

    mark[atom] = TRUE;
    t = system->atoms[atom].type;
    if ( ignore == TRUE && control->ignore[t] ) {
        return;
    }
    type[t]++;
    ...
    for ( i = start; i < end; ++i ) {
        ...
        if ( bo >= control->bg_cut && mark[nbr] == FALSE )
...
static void Analyze_Fragments( reax_system *system, control_params *control,
        simulation_data *data, static_storage *workspace,
        reax_list **lists, FILE *fout, bool ignore )
{
    uint32_t atom, i;
    bool flag;
    bool *mark = workspace->mark;
    uint32_t num_fragments, num_fragment_types;
    char fragment[MAX_ATOM_TYPES];
    char fragment_out[MAX_ATOM_TYPES];
    char fragments[MAX_FRAGMENT_TYPES][MAX_ATOM_TYPES];
    uint32_t fragment_count[MAX_FRAGMENT_TYPES];
    molecule m;
    reax_list *new_bonds = lists[BONDS];
...
static void Analyze_Silica( reax_system *system, control_params *control,
                     ...
                     reax_list **lists, FILE *fout )
{
    uint32_t atom, i, j, k, pi, pk, pk_j, newp, coord;
    uint32_t O_SI_O_count, SI_O_SI_count;
    uint32_t si_coord[10], ox_coord[10];
    real O_SI_O, SI_O_SI;
    reax_list *new_bonds = lists[BONDS];
    reax_list *thb_intrs = lists[THREE_BODIES];

    Analyze_Fragments( system, control, data, workspace, lists, fout, FALSE );
...
static uint32_t Get_Type_of_Molecule( molecule *m )
{
        ...
        reax_list *bonds, FILE *fout )
{
    ...
    for ( atom = 0; atom < system->N; ++atom ) {
        ...
        if ( mark[atom] == FALSE && system->atoms[atom].type == 2 ) {
            rvec_MakeZero( mu );

            memset( &m, 0, sizeof(molecule) );
            Get_Molecule( atom, &m, mark, system, control, bonds, false, fout );
        ...
        FILE *fout )
{
    ...
    cell_counter = (uint32_t ***) scalloc( mesh_dims[0], sizeof(uint32_t),
           "Calculate_Density_3DMesh::cell_counter" );

    for ( i = 0; i < mesh_dims[0]; ++i ) {
        cell_counter[i] = (uint32_t **) scalloc( mesh_dims[1], sizeof(uint32_t),
               "Calculate_Density_3DMesh::cell_counter[i]" );

        for ( j = 0; j < mesh_dims[1]; ++j ) {
            cell_counter[i][j] = (uint32_t *) scalloc( mesh_dims[2], sizeof(uint32_t),
...
static void Calculate_Density_Slice( reax_system *system, simulation_data *data, FILE *fout )
{
    ...
    uint32_t *slice_occ;
    uint32_t i, num_slices, my_slice, max_occ = 0;

    /* allocate counter */
    num_slices = system->box.box_norms[2] / slice_thickness + 1.0;
    slice_occ = (uint32_t*) scalloc( num_slices, sizeof(uint32_t),
