...
void Hip_Compute_NonBonded_Forces_Part1( reax_system const * const system,
...
#if defined(LOG_PERFORMANCE)
    hipEventRecord( control->hip_time_events[TE_VDW_STOP], control->hip_streams[4] );
#endif
#endif
}


void Hip_Compute_NonBonded_Forces_Part2( reax_system const * const system,
        control_params const * const control, simulation_data * const data,
        storage * const workspace, reax_list **lists,
        output_controls const * const out_control )
{
...
#if defined(LOG_PERFORMANCE)
    hipEventRecord( control->hip_time_events[TE_COULOMB_START], control->hip_streams[5] );
#endif
...
#if defined(LOG_PERFORMANCE)
    hipEventRecord( control->hip_time_events[TE_COULOMB_STOP], control->hip_streams[5] );
#endif
}
