...
GPU_DEVICE static inline real Hip_DistSqr_to_Special_Point( rvec cp, rvec x ) 
{
    int i;  
    real d_sqr = 0.0;
        ...
        int * const realloc_far_nbrs, real cutoff2 )
{
    ...
    { 
        ...
        if ( Hip_DistSqr_to_Special_Point(g.nbrs_cp[index_grid_nbrs(i, j, k, itr, &g)], x) <= cutoff )
        {
        ...
        reax_list **lists )
{
    ...
    hipEventRecord( control->hip_time_events[TE_NBRS_START], control->hip_streams[0] );
#endif
...
//    k_generate_neighbor_lists_full <<< blocks, NBRS_BLOCK_SIZE, 0, control->hip_streams[0] >>>
...
//    k_estimate_neighbors_full <<< blocks, DEF_BLOCK_SIZE, 0, control->hip_streams[0] >>>
...
//          system->d_far_nbrs, system->d_max_far_nbrs, SQR( control->bond_cut ) );
    ...
    Hip_Reduction_Sum( system->d_max_far_nbrs, system->d_total_far_nbrs,
            system->total_cap, 0, control->hip_streams[0] );
