...
void Hip_Reset_Workspace( reax_system const * const system,
...
void Hip_Reset_Atoms_HBond_Indices( reax_system * const system,
        control_params const * const control, storage * const workspace )
{
#if defined(GPU_KERNEL_ATOMIC)
...
#if !defined(GPU_KERNEL_ATOMIC)
    Hip_Reduction_Sum( hindex, system->d_num_H_atoms, system->N, 0,
