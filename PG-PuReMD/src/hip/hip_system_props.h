
#ifndef __HIP_SYSTEM_PROPS_H__
#define __HIP_SYSTEM_PROPS_H__

#include "../reax_types.h"


void Hip_Generate_Initial_Velocities( reax_system * const,
        control_params const * const, real );

void Hip_Compute_Total_Mass( reax_system * const, control_params const * const,
        storage * const, simulation_data * const, MPI_Comm );

void Hip_Compute_Pressure( reax_system * const, control_params const * const,
        storage * const, simulation_data * const, mpi_datatypes * const );

#ifdef __cplusplus
extern "C"  {
#endif

void Hip_Compute_Kinetic_Energy( reax_system * const, control_params const * const,
        storage * const, simulation_data * const, MPI_Comm );

void Hip_Compute_Center_of_Mass( reax_system * const, control_params const * const,
        storage * const, simulation_data * const, mpi_datatypes * const, MPI_Comm );

#ifdef __cplusplus
}
#endif


#endif
