...
static void Hip_Compute_Momentum( reax_system * const system,
        ...
        storage * const workspace, rvec xcm, rvec vcm, rvec amcm )
{
    ...
    sHipMemcpyAsync( xcm,
            ...
            &spad[blocks],
#endif
            sizeof(rvec), hipMemcpyDeviceToHost, control->hip_streams[0],
            __FILE__, __LINE__ );
...
static void Hip_Compute_Inertial_Tensor( reax_system * const system,
...
extern "C" void Hip_Compute_Kinetic_Energy( reax_system * const system,
        control_params const * const control, storage * const workspace,
        simulation_data * const data, MPI_Comm comm )
{
    ...
    sHipMemcpyAsync( &data->my_en[E_KIN], &kinetic_energy[system->n],
            sizeof(real), hipMemcpyDeviceToHost, control->hip_streams[0],
            __FILE__, __LINE__ );
    hipStreamSynchronize( control->hip_streams[0] );
...
void Hip_Compute_Total_Mass( reax_system * const system,
        control_params const * const control,
...
extern "C" void Hip_Compute_Center_of_Mass( reax_system * const system,
        control_params const * const control, storage * const workspace,
 ...
 */
void Hip_Compute_Pressure( reax_system * const system,
        control_params const * const control,
