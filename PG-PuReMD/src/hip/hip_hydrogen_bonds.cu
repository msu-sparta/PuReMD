...
//    int hbs, hnbrs_blocks;
    ...
    hipEventRecord( control->hip_time_events[TE_HBONDS_START], control->hip_streams[2] );
#endif
    ...
    hipStreamWaitEvent( control->hip_streams[2], control->hip_stream_events[SE_BOND_ORDER_DONE], 0 );
...
//                );
    ...
    {
        Hip_Reduction_Sum( spad, &data->d_my_en[E_HB], system->n, 2,
                control->hip_streams[2] );
    }
    ...
    if ( control->virial == 1 )
    {
        ...
        Hip_Reduction_Sum( rvec_spad, &data->d_my_ext_press, system->n, 2,
                control->hip_streams[2] );
    }
#endif
...
#if defined(LOG_PERFORMANCE)
    hipEventRecord( control->hip_time_events[TE_HBONDS_STOP], control->hip_streams[2] );
#endif
}
