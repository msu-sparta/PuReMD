...
void GPU_Scale_Box( reax_system *system, control_params *control,
        storage *workspace, simulation_data *data, mpi_datatypes *mpi_data )
{
    ...
    GPU_Scale_Velocities_NPT( system, control, lambda, mu );

    GPU_Compute_Kinetic_Energy( system, control, workspace, data, mpi_data->comm_mesh3D );
