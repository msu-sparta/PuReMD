...
typedef void (*gpu_dist_packer)( void const * const, mpi_out_data * const,
        int, cudaStream_t );
typedef void (*gpu_coll_unpacker)( void const * const, void * const,
...
static gpu_dist_packer Get_Packer( int type )
{
    gpu_dist_packer func_ptr;
...
static gpu_coll_unpacker Get_Unpacker( int type )
{
    gpu_coll_unpacker func_ptr;
...
void GPU_Dist( reax_system const * const system, storage * const workspace,
...
//    type_size = MPI_Aint_add( lower_bound, extent );
	...
	if ( out_bufs[2 * d + 1].index_size < sizeof(int) * out_bufs[2 * d + 1].cnt )
	{
        ...
        {
            fprintf( stderr, "[ERROR] GPU_Dist: not enough space in recv buffer for nbr1 (dim = %d)\n", d );
            MPI_Abort( MPI_COMM_WORLD, RUNTIME_ERROR );
        }
        ...
        {
            fprintf( stderr, "[ERROR] GPU_Dist: not enough space in recv buffer for nbr2 (dim = %d)\n", d );
            MPI_Abort( MPI_COMM_WORLD, RUNTIME_ERROR );
        }
...
void GPU_Coll( reax_system const * const system, mpi_datatypes * const mpi_data,
