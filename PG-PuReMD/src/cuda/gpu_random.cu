...
GPU_DEVICE double GPU_Random( double range )
{
    //TODO: use cuRAND
...
GPU_DEVICE double GPU_GRandom( double mean, double sigma )
{
    double v1 = GPU_Random(2.0) - 1.0;
    double v2 = GPU_Random(2.0) - 1.0;
    double rsq = v1 * v1 + v2 * v2;

    while (rsq >= 1.0 || rsq == 0.0)
    {
        v1 = GPU_Random(2.0) - 1.0;
        v2 = GPU_Random(2.0) - 1.0;
        rsq = v1 * v1 + v2 * v2;
    }
