        ...
        output_controls const * const out_control )
{
    ...
    cudaEventRecord( control->gpu_time_events[TE_BONDS_START], control->gpu_streams[1] );
#endif
    ...
    cudaStreamWaitEvent( control->gpu_streams[1], control->gpu_stream_events[SE_BOND_ORDER_DONE], 0 );
...
#if defined(LOG_PERFORMANCE)
    cudaEventRecord( control->gpu_time_events[TE_BONDS_STOP], control->gpu_streams[1] );
#endif
}
