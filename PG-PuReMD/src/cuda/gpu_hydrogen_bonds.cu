...
//    int hbs, hnbrs_blocks;
    ...
    cudaEventRecord( control->gpu_time_events[TE_HBONDS_START], control->gpu_streams[2] );
#endif
    ...
    cudaStreamWaitEvent( control->gpu_streams[2], control->gpu_stream_events[SE_BOND_ORDER_DONE], 0 );
...
#if defined(LOG_PERFORMANCE)
    cudaEventRecord( control->gpu_time_events[TE_HBONDS_STOP], control->gpu_streams[2] );
#endif
}
