...
CUDA_GLOBAL void k_vdW_coulomb_energy( reax_atom *my_atoms, 
        ...
        storage workspace, reax_list far_nbr_list, int n, int N, int num_atom_types, 
    ...
    if ( i < N )
    {
        ...
        while ( pj < end_i )
        {
            ...
            if ( far_nbr_list.far_nbr_list.d[pj] <= control->nonb_cut
                    && (((i < j) && (i < n) && (j < n || orig_i < orig_j))
                        || ((i > j) && (i < n) && (j < n))
                        || (i > j && i >= n && j < n && orig_j < orig_i)))
            {
...
#if defined(__SM_35__)
                    sh_vdw += (self_coef * (e_core * Tap)) / 2.0;
#else
                    sh_vdw[ threadIdx.x ] += (self_coef * (e_core * Tap)) / 2.0;
#endif
...
{
    int blocks, rblocks, update_energy, size;
    rvec *spad_rvec;
    real *spad;

    size = (2 * system->N + 2 * system->N ) * sizeof(real)
    ...
    cuda_memset( spad, 0, size, "Cuda_NonBonded_Energy::spad" );
