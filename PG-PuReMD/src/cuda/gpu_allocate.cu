...
static void GPU_Reallocate_List( reax_list * const list, size_t n, size_t max_intrs,
        int type )
{
    GPU_Delete_List( list );
    GPU_Make_List( n, max_intrs, type, list );
}


static void GPU_Reallocate_System_Part1( reax_system * const system,
        ...
        int local_cap_old )
{
    ...
    sCudaMemcpyAsync( temp, system->d_cm_entries, sizeof(int) * local_cap_old,
            cudaMemcpyDeviceToDevice, control->gpu_streams[0], __FILE__, __LINE__ );
...
static void GPU_Reallocate_System_Part2( reax_system * const system,
        control_params const * const control, storage * const workspace,
        int total_cap_old )
{
    int *temp;
...
void GPU_Allocate_Grid( reax_system * const system,
...
void GPU_Deallocate_Grid_Cell_Atoms( reax_system * const system,
        control_params const * const control )
{
    int i, total;
    grid_cell local_cell;
...
void GPU_Allocate_Grid_Cell_Atoms( reax_system * const system,
...
void GPU_Allocate_System( reax_system * const system,
        control_params const * const control )
{
    /* atoms */
    sCudaMalloc( (void **) &system->d_my_atoms,
...
void GPU_Allocate_Simulation_Data( simulation_data * const data, cudaStream_t s )
{
...
void GPU_Allocate_Workspace_Part1( control_params const * const control, 
...
void GPU_Allocate_Workspace_Part2( control_params const * const control, 
...
void GPU_Deallocate_Workspace_Part1( control_params const * const control,
...
void GPU_Deallocate_Workspace_Part2( control_params const * const control,
        storage * const workspace )
{
    /* bond order related storage  */
 ...
 */
void GPU_Allocate_Matrix( sparse_matrix * const H, int n, int n_max, int m,
...
void GPU_Deallocate_Matrix( sparse_matrix * const H )
{
    H->allocated = FALSE;
    H->n = 0;
    H->n_max = 0;
...
void GPU_Reallocate_Part1( reax_system * const system,
        control_params const * const control,
...
//        GPU_Deallocate_Grid_Cell_Atoms( system );
//        GPU_Allocate_Grid_Cell_Atoms( system, realloc[RE_GCELL_ATOMS] );
        realloc[RE_GCELL_ATOMS] = -1;
    }
}


void GPU_Reallocate_Part2( reax_system * const system,
        control_params const * const control,
        simulation_data * const data, storage * const workspace,
        reax_list ** const lists, mpi_datatypes * const mpi_data )
{
    ...
    if ( nflag == TRUE )
    {
        GPU_Reallocate_System_Part1( system, control, workspace, local_cap_old );

        GPU_Deallocate_Workspace_Part1( control, workspace->d_workspace );
        GPU_Allocate_Workspace_Part1( control, workspace->d_workspace,
                system->local_cap );
    }

    if ( Nflag == TRUE )
    {
        GPU_Reallocate_System_Part2( system, control, workspace, total_cap_old );

        GPU_Deallocate_Workspace_Part2( control, workspace->d_workspace );
        GPU_Allocate_Workspace_Part2( control, workspace->d_workspace,
                system->total_cap );
    }
    ...
    {
        GPU_Reallocate_List( lists[FAR_NBRS], system->total_cap,
    ...
    if ( nflag == TRUE || realloc[RE_CM] == TRUE )
    {
        ...
        GPU_Deallocate_Matrix( H );
        GPU_Allocate_Matrix( H, system->n, system->local_cap,
                system->total_cm_entries, format, control->gpu_streams[0] );
    ...
    {
        GPU_Reallocate_List( lists[BONDS], system->total_cap,
    ...
    {
        GPU_Reallocate_List( lists[HBONDS], system->total_cap,
    ...
    {
        GPU_Reallocate_List( lists[THREE_BODIES], system->total_thbodies_indices,
