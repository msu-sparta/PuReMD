            ...
            for ( ci[2] = 0; ci[2] < g->ncells[2]; ci[2]++ )
            {
                ...
                for ( cj[0] = cmin[0]; cj[0] < cmax[0]; ++cj[0] )
                    for ( cj[1] = cmin[1]; cj[1] < cmax[1]; ++cj[1] )
                        ...
                        {
                            //fprintf( stderr, "\tgrid2: %d %d %d - ", cj[0], cj[1], cj[2] );
                            gc->nbrs[top] = &(g->cells[cj[0]][cj[1]][cj[2]]);
                            ivec_Copy( gc->nbrs_x[top], cj );
                            Find_Closest_Point( /*ext_box,*/ g, ci, cj, gc->nbrs_cp[top] );
...
void Bin_Boundary_Atoms( reax_system *system )
{
...
#if defined(DEBUG)
    fprintf( stderr, "p%d bin_boundary_atoms: entered\n", system->my_rank );
#endif
    g = &(system->my_grid);
    atoms = system->my_atoms;
    start = system->n;
    ...
    Get_Boundary_GCell( g, base, atoms[start].x, &gc, &cur_min, &cur_max );
    gc->str = start;
    gc->top = 1;
    /* error check */
    ...
    for ( i = start + 1; i < end; i++ )
    {
        ...
        {
            gc->end = i;
            Get_Boundary_GCell( g, base, atoms[i].x, &gc, &cur_min, &cur_max );
            /* sanity check! */
            if ( gc->top != 0 )
            {
