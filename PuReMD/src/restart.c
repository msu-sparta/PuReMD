			   ...
			   mpi_datatypes *mpi_data )
{
  ...
  if( me == MASTER_NODE ) {
    ...
    buffer = (restart_atom*)
      scalloc( system->bigN, sizeof(restart_atom), "restart:buffer", comm );
  }
  else
    buffer = (restart_atom*)
      scalloc( system->n, sizeof(restart_atom), "restart:buffer", comm );
		    ...
		    mpi_datatypes *mpi_data )
{
  ...
  fres = NULL;
  line = (char*) 
    smalloc( sizeof(char) * RESTART_LINE_LEN, "restart:line", comm );
  me = system->my_rank;
  np = control->nprocs;
  ...
  buffer = (char*) smalloc( sizeof(char) * buffer_req, "restart:buffer", comm );
  line[0] = 0;
  buffer[0] = 0;
		   ...
		   storage *workspace, mpi_datatypes *mpi_data )
{
  ...
  fgets( s, MAX_LINE, fres );
  c = Tokenize( s, &tmp );
  box[0][0] = atof(tmp[0]); box[0][1] = atof(tmp[1]); box[0][2] = atof(tmp[2]);
  fgets( s, MAX_LINE, fres );
  c = Tokenize( s, &tmp );
  box[1][0] = atof(tmp[0]); box[1][1] = atof(tmp[1]); box[1][2] = atof(tmp[2]);
  fgets( s, MAX_LINE, fres );
  c = Tokenize( s, &tmp );
  box[2][0] = atof(tmp[0]); box[2][1] = atof(tmp[1]); box[2][2] = atof(tmp[2]);
  ...
  for( i = 0; i < system->bigN; ++i ) {
    ...
    x_temp[0]=atof(tmp[3]); x_temp[1]=atof(tmp[4]); x_temp[2]=atof(tmp[5]);
    v_temp[0]=atof(tmp[6]); v_temp[1]=atof(tmp[7]); v_temp[2]=atof(tmp[8]);
